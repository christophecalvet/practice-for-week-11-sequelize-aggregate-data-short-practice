...
app.get('/cats/:id/toys', async (req, res, next) => {
    ...
    // display `catData`
   // res.json({ catToysAggregateData, cat }); //Pre-refactor
   res.json(catData)
})
...
app.get('/data-summary', async (req, res, next) => {
   let dataSummary = {}
    dataSummary.totalNumberOfCat = await Cat.count();
    dataSummary.totalNumberOfToys = await Toy.count();

    const toysSummary =  await Toy.findAll( {
        attributes: [
                [ sequelize.fn("AVG", sequelize.col("price")), "avgUseraveragePriceOfAToy"],
    ...
    dataSummary.toysSummary = toysSummary
    ...
    dataSummary.expensiveToySummary = expensiveToySummary;
